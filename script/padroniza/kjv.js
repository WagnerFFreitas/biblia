...
window.BIBLE_VERSION = 'kjv';                                                                         // Identificador da versão KJV
window.NOME_VERSAO_COMPLETA_BIBLIA = 'King James Version';                                                          // Nome completo da tradução
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para KJV.`);                  // Log de inicialização
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {                                                 // Função principal de carregamento
    console.log(`[KJV] Carregando: ${livro} ${capitulo}:${versiculo}`);                                                 // Log do versículo sendo carregado
    
    const content = document.querySelector('.conteudo');                                                                // Localiza área de conteúdo
    if (!content) {                                                                                                     // Validação da área de conteúdo
        console.error("[KJV] Elemento .conteudo não encontrado.");                                                      // Log de erro
    ...
    try {                                                                                 // Inicia tratamento de erro
        const response = await fetch(`../versao/kjv/${livro}/${capitulo}.json`);                                        // Requisita arquivo JSON
        if (!response.ok) {                                                                                             // Valida resposta HTTP
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (KJV)`);                  // Lança erro de requisição
        }
        ...
        if (data.versiculos && data.versiculos[versiculo]) {                                                            // Verifica se versículo existe
            ...
            console.warn(`[KJV] Versículo ${versiculo} não encontrado em ${livro} ${capitulo}.json (KJV)`);             // Log de aviso
        }
    } catch (error) {                                                                                                   // Captura erros
        console.error(`[KJV] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (KJV):`, error);              // Log de erro
        const textoP = document.createElement('p');                                                                     // Cria parágrafo de erro
    ...
    if (window.titulo) {                                                                                                // Verifica se título existe
        ...
        if (typeof window.getLivroDisplayName === 'function') {                                                         // Verifica função de nome
            ...
            console.warn("[KJV] Função getLivroDisplayName não encontrada.");                                           // Log de aviso
        }
        ...
        console.warn(`[KJV] Elemento H2 principal (window.titulo) não encontrado.`);                                    // Log de aviso
    }
}

window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {         // Função para obter título
    console.log(`[KJV] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);                                // Log de busca
    try {                                                                                                               // Inicia tratamento de erro
        const response = await fetch(`../versao/kjv/${livro}/${capitulo}.json`);                                        // Requisita arquivo JSON
        if (!response.ok) {                                                                                             // Valida resposta HTTP
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (KJV)`);                  // Lança erro de requisição
        ...
        console.error(`[KJV] Erro ao obter título interno para ${livro} ${capitulo}:${versiculo} (KJV):`, error);       // Log de erro
