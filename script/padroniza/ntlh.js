...
window.BIBLE_VERSION = 'ntlh';                                                                   // Identificador da versão NTLH
window.NOME_VERSAO_COMPLETA_BIBLIA = 'Nova Tradução na Linguagem de Hoje';                                     // Nome completo da tradução
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para NTLH.`);            // Log de inicialização
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {                                            // Função principal de carregamento
    console.log(`[NTLH] Carregando: ${livro} ${capitulo}:${versiculo}`);                                           // Log do versículo sendo carregado
    
    const content = document.querySelector('.conteudo');                                                           // Localiza área de conteúdo
    if (!content) {                                                                                                // Validação da área de conteúdo
        console.error("[NTLH] Elemento .conteudo não encontrado.");                                                // Log de erro
    ...
    try {                                                                                 // Inicia tratamento de erro
        const response = await fetch(`../versao/ntlh/${livro}/${capitulo}.json`);                                  // Requisita arquivo JSON
        if (!response.ok) {                                                                                        // Valida resposta HTTP
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NTLH)`);            // Lança erro de requisição
        }
        ...
        if (data.versiculos && data.versiculos[versiculo]) {                                                       // Verifica se versículo existe
            ...
            console.warn(`[NTLH] Versículo ${versiculo} não encontrado em ${livro} ${capitulo}.json (NTLH)`);      // Log de aviso
        }
    } catch (error) {                                                                                              // Captura erros
        console.error(`[NTLH] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NTLH):`, error);       // Log de erro
        const textoP = document.createElement('p');                                                                // Cria parágrafo de erro
    ...
    if (window.titulo) {                                                                                           // Verifica se título existe
        ...
        if (typeof window.getLivroDisplayName === 'function') {                                                    // Verifica função de nome
            ...
            console.warn("[NTLH] Função getLivroDisplayName não encontrada.");                                     // Log de aviso
        }
        ...
        console.warn(`[NTLH] Elemento H2 principal (window.titulo) não encontrado.`);                              // Log de aviso
    }
}

window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {         // Função para obter título
    console.log(`[NTLH] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);                          // Log de busca
    try {                                                                                                          // Inicia tratamento de erro
        const response = await fetch(`../versao/ntlh/${livro}/${capitulo}.json`);                                  // Requisita arquivo JSON
        if (!response.ok) {                                                                                        // Valida resposta HTTP
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NTLH)`);            // Lança erro de requisição
        ...
        console.error(`[NTLH] Erro ao obter título interno para ${livro} ${capitulo}:${versiculo} (NTLH):`, error);// Log de erro
