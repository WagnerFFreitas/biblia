...
window.BIBLE_VERSION = 'acf';                                                              // Identificador da versão ACF
window.NOME_VERSAO_COMPLETA_BIBLIA = 'Almeida Corrigida Fiel';                             // Nome completo da tradução
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções para ACF.`); // Log de inicialização
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {                    // Função principal de carregamento
    console.log(`[ACF] Carregando: ${livro} ${capitulo}:${versiculo}`);                    // Log do versículo sendo carregado
    
    const content = document.querySelector('.conteudo');                                   // Localiza área de conteúdo
    if (!content) {                                                                        // Validação da área de conteúdo
        console.error("[ACF] Elemento .conteudo não encontrado.");                         // Log de erro
    ...
    try {                                                                             // Inicia tratamento de erro
        const response = await fetch(`../versao/acf/${livro}/${capitulo}.json`);     // Requisita arquivo JSON
        if (!response.ok) {                                                           // Valida resposta HTTP
            throw new Error(`HTTP ${response.status} ao buscar JSON para ACF`);      // Lança erro de requisição
        }
        ...
        if (data.versiculos && data.versiculos[versiculo]) {                         // Verifica se versículo existe
            ...
            console.warn(`[ACF] Versículo não encontrado em ${livro} ${capitulo}.json`); // Log de aviso
        }
    } catch (error) {                                                               // Captura erros
        console.error(`[ACF] Erro ao carregar versículo (ACF):`, error);           // Log de erro
        const textoP = document.createElement('p');                                 // Cria parágrafo de erro
    ...
    if (window.titulo) {                                                                   // Verifica se título existe
        ...
        if (typeof window.getLivroDisplayName === 'function') {                            // Verifica função de nome
            ...
            console.warn("[ACF] Função getLivroDisplayName não encontrada.");              // Log de aviso
        }
        ...
        console.warn(`[ACF] window.titulo não encontrado.`);                               // Log de aviso
    }
}

window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {              // Função para obter título
    console.log(`[ACF] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);   // Log de busca
    try {                                                                                  // Inicia tratamento de erro
        const response = await fetch(`../versao/acf/${livro}/${capitulo}.json`);           // Requisita arquivo JSON
        if (!response.ok) {                                                                // Valida resposta HTTP
            throw new Error(`HTTP ${response.status} ao buscar JSON para ACF`);            // Lança erro de requisição
        ...
        console.error(`[ACF] Erro ao obter título interno (ACF):`, error);                 // Log de erro
