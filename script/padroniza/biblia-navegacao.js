...
/* BLOCO: Explicação introdutória sobre o sistema de navegação */
// Este arquivo é como o "cérebro" da navegação da Bíblia digital.
// Ele controla o que acontece quando você clica em um livro, capítulo ou versículo.
// É como um sistema de menus que organiza todo o conteúdo bíblico de forma interativa.

console.log("[biblia-navegacao.js] Script carregado.");                           
// Esta linha escreve uma mensagem no console do navegador (F12) confirmando que este arquivo foi carregado com sucesso.
// É útil para programadores identificarem problemas se algo não estiver funcionando.

/* BLOCO: Tabela de informações dos livros bíblicos */
// Este bloco é como uma "tabela de informações" sobre todos os livros da Bíblia.
// Para cada livro, guardamos:
// - A "chave" (nome interno usado pelo programa, como "genesis")
// - Quantos capítulos o livro tem (ex: Gênesis tem 50 capítulos)
// - O nome bonito para mostrar na tela (ex: "GÊNESIS" em maiúsculas)
// É como ter uma lista telefônica, mas para livros bíblicos!
...
window.titulo = null;                                                             
// Armazena a referência ao elemento H2 que mostra o título atual (ex: "GÊNESIS - CAPÍTULO 1").

window.activeLivro = null;                                                        
// Armazena a chave do livro atualmente selecionado (ex: "genesis").

window.activeCapitulo = null;                                                     
// Armazena o número do capítulo atualmente selecionado (ex: 1).

window.activeVersiculoButton = null;                                              
// Armazena a referência ao botão do versículo atualmente ativo, para controle de estilo e comportamento.
...
window.getLivroDisplayName = function(livroKey) {
    if (livros[livroKey] && livros[livroKey].displayName) {                       
        // Verifica se a chave do livro existe na configuração e tem um nome de exibição.
        return livros[livroKey].displayName;                                      
        // Retorna o nome formatado (ex: "GÊNESIS").
    }
    return livroKey ? livroKey.toUpperCase() : "LIVRO DESCONHECIDO";              
    // Se não encontrar, retorna a chave em maiúsculas ou uma mensagem de erro.
};
