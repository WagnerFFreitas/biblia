...
window.BIBLE_VERSION                   = 'ntlh';                                                                   /* Identificador da versão      */
window.NOME_VERSAO_COMPLETA_BIBLIA     = 'Nova Tradução na Linguagem de Hoje';                                     /* Nome completo da tradução    */
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para NTLH.`);            /* Log de carga do módulo       */
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {                                            /* Inicia motor de carga        */
    console.log(`[NTLH] Carregando: ${livro} ${capitulo}:${versiculo}`);                                           /* Log de posição de leitura    */
    const content = document.querySelector('.conteudo');                                                           /* Localiza contêiner no DOM    */
    if (!content) {                                                                                                /* Valida existência elemento   */
        console.error("[NTLH] Elemento .conteudo não encontrado.");                                                /* Relata falha de interface    */
        return;
    }
    ...
    try {
        const response = await fetch(`../versao/ntlh/${livro}/${capitulo}.json`);                                  /* Solicita arquivo ao servidor */
        if (!response.ok) {                                                                                        /* Valida resposta da rede      */
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NTLH)`);            /* Lança exceção de falha       */
        }
        ...
        if (data.versiculos && data.versiculos[versiculo]) {                                                       /* Verifica se índice existe    */
            ...
            console.warn(`[NTLH] Versículo ${versiculo} não encontrado em ${livro} ${capitulo}.json (NTLH)`);      /* Alerta técnico no console    */
        }
        ...
        console.error(`[NTLH] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NTLH):`, error);       /* Log de erro crítico técnico  */
    ...
    if (window.titulo) {                                                                                           /* Verifica se H2 está pronto   */
        ...
        if (typeof window.getLivroDisplayName === 'function') {                                                    /* Verifica tradutor de nomes   */
            ...
            console.warn("[NTLH] Função getLivroDisplayName não encontrada.");                                     /* Alerta falta de função       */
        }
        window.titulo.textContent = `${nomeLivroDisplay} - CAPÍTULO ${capitulo} - VERSÍCULO ${versiculo}`;         /* Escreve novo título H2       */
    } else {
        console.warn(`[NTLH] Elemento H2 principal (window.titulo) não encontrado.`);                              /* Alerta ausência de H2        */
    }
}
...
window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {                                      /* Inicia busca de subtítulo    */
    console.log(`[NTLH] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);                          /* Log de busca de metadados    */
    try {
        const response = await fetch(`../versao/ntlh/${livro}/${capitulo}.json`);                                  /* Solicita dados do servidor   */
        if (!response.ok) {                                                                                        /* Valida resposta da rede      */
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NTLH)`);            /* Lança exceção de erro        */
        }
        const data = await response.json();                                                                        /* Converte em objeto real      */
        return data.titulos && data.titulos[versiculo] ? data.titulos[versiculo] : null;                           /* Retorna subtítulo ou nulo    */
    } catch (error) {
        console.error(`[NTLH] Erro ao obter título interno para ${livro} ${capitulo}:${versiculo} (NTLH):`, error);/* Log de falha na busca        */
